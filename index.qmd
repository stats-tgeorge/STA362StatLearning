...
library(tidyverse)
gs4_deauth()
d <- read_sheet("https://docs.google.com/spreadsheets/d/1VC0F31pzRgeXaIQMgZlWZSfAJbyCxo8iryWrxJfseBU/edit?usp=sharing")
```
...
d %>% 
  ...
  rename(Day = day, Date = date, Topic = topic,Notes = notes,
         `Notes 2` = notes2,`Outline 2` = outline2,
  ...
  text_transform(cells_body(columns = c(Outline)),
    ...
    text_transform(cells_body(columns = c(`Notes 2`)),
               fn = function(x){
                 ifelse(is.na(x), fontawesome::fa('laptop', fill_opacity = 0.1), 
                        glue::glue("<a href = {x}>{fontawesome::fa('laptop')}</a>"))
               }) %>%
    text_transform(cells_body(columns = c(`Outline 2`)),
               fn = function(x){
                 ifelse(is.na(x), fontawesome::fa('list', fill_opacity = 0.1), 
                        glue::glue("<a href = {x}>{fontawesome::fa('list')}</a>"))
               }) %>%
