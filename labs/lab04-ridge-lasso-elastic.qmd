...
For this lab, we are using a data frame currently in `music.csv`. You will need to upload it to the server or copy it into your project directory (see Moodle). This data frame includes 72 predictors that are components of audio files and one outcome, `lat`, the latitude of where the music originated. We are trying to predict the location of the music's origin using audio components of the music.

[Download the music.CSV](labs/data/music.csv)
