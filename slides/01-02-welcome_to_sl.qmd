...
format: 
  revealjs:
    output-file: "01-02-welcome_to_sl.html"
    slide-number: true
    echo: false
    message: false
    warning: false
  html:
    output-file: "01-02-welcome_to_sl_o.html"
editor_options: 
...
* Take the average! $f(100) = E[\texttt{mpg}|\texttt{horsepower} = 100] = \hat{f}(X_h=100)=19.6$
...
> * $\hat\varepsilon = Y - \hat{f}(X_h=100) = 32.9 - 19.6 = \color{red}{13.3}$
...
::: question
I have some *training* data, plotted above. What $\hat{f}(x)$ would minimize the $MSE_{\texttt{train}}$?
:::
...
> * The _true_ model is $Y = f(x) + \varepsilon$ and $f(x) = E[Y|X=x]$
...
$$Err_{\texttt{test}} = \frac{\#incorrect predictions}{total predictions} = \textrm{Ave}_{test}I[y_i\neq \mathcal{\hat{C}}(x_i)]$$
