...
- With Naive Bayes we drop the normality assumption but introduce a much stronger assumption
  -   Within a class $k$, the $p$ predictors are independent
  
- Stated mathematically $f_x(x) = f_{k1}(x_1)*f_{k2}(x_2)\times\cdot\cdot\cdot \times f_{kp}(x_p)$
...
Making these assumptions, we now have:
...
nb_fit_1 <- naive_Bayes() |> # The default predictor assumption is none
  # it will use smoothing (kernel density) to get the job done
  # setting usekernel = T will assume predictors are normal 
  set_mode("classification")|>
  set_args(usekernel=FALSE)|> # now the assumption is normal
  set_engine("klaR")|> 
    fit(default ~ balance + student,
      data = default_training)
