...
format: 
  revealjs:
    output-file: "05-cv-tidymodels.html"
    slide-number: true
  html:
    output-file: "05-cv-tidymodels_o.html"
editor_options: 
...
::: question
If we have a **quantitative response** what metric would we use to calculate this test error?
:::
...
::: question
If we have a **qualitative response** what metric would we use to calculate this test error?
:::
...
* The premise is the same as cross validation for quantitative outcomes
* Split the data into K parts, where $C_1, C_2, \dots, C_k$ indicate the indices of observations in part $k$
...
Write a pipe that creates a model that uses *linear_reg()* and engine *lm* to fit a linear regression using tidymodels. Save it as *lm_spec* and look at the object. What does it return?

:::

_Hint: you'll need  <https://www.tidymodels.org>_
...
2. Does this give the same results as
