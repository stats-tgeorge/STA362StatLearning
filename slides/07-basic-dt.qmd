...
## <i class="fas fa-laptop"></i> `Exercise`
...
1) Use **recursive binary splitting** to grow a large tree on the training data, stop when you reach some stopping criteria (such as there is some minimum number of observations in each region)


2) Apply **cost complexity pruning** to the larger tree to obtain a sequence of best subtrees, as a function of $\alpha$


3) Use K-fold cross-validation to choose $\alpha$. Pick $\alpha$ to minimize the average error
